...
export default function DivisionList() {
  ...
  // }, [selectDivId])
  useEffect(() => {
    axios.get(`${BASE_URL}/department/list`, {
      headers: {
        Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}`
      }
    }).then((res) => {
          ...
          ? <S.WorkHomeReqBg>
            <S.WorkHomeReq>
              <S.ModalHeader>
                ...
                <SvgCross onClick={() => { setAddDivModal(false) }} style={{ cursor: "pointer" }} width="36" height="36" fill="white" />
              </S.ModalHeader>
              <S.ModalContent>
                <S.DateSetting>
                  <h1>이름</h1>
                  <S.ModalDateInput onChange={(e) => { setAddDivName(e.target.value) }} type={"text"} />
                </S.DateSetting>
                <S.DateSetting>
                  <h1>위치</h1>
                  <S.ModalDateInput onChange={(e) => { setAddDivLoc(e.target.value) }} type={"text"} />
                </S.DateSetting>
                <S.ModalReqBtnWrap>
                  <S.ModalReqBtn onClick={() => { useAddDivision(addDivName, addDivLoc) }}>추가</S.ModalReqBtn>
                </S.ModalReqBtnWrap>
              </S.ModalContent>
            </S.WorkHomeReq>
          </S.WorkHomeReqBg> : null
      }
      <S.ContentWrap>
        <S.DivListWrap>
          <S.DivList>
            <S.Division onClick={() => { setAddDivModal(true) }} style={{ backgroundColor: "#E9E9E9", alignItems: "center", justifyContent: "center" }}>
          ...
          </S.DivList>
          {
            divList?.map((data, key) => (
              <S.DivList key={key}>
                <S.Division onClick={() => { setSelectDivId(data.department_id) }}>
